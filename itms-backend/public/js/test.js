...
searchBtn.addEventListener("click", ()=>{
    ...
    if(searchBtn.innerText === 'Search'){
        if(departmentInput.value === 'none' && designationInput.value === 'none'){
            alert("Please select either a department or a designation")
        }else{
            searchParams.department = departmentInput.value
            searchParams.designation = designationInput.value
            searchBtn.innerText = 'Clear'
            searchBtn.classList.replace("btn-primary", "btn-danger")
            loadInitialTableData()
        }
    }else{
        searchParams.department = "none"
        searchParams.designation = "none"
        designationInput.value = "none"
        departmentInput.value = "none"
        searchBtn.innerText = 'Search'
        searchBtn.classList.replace("btn-danger", "btn-primary")
        loadInitialTableData()
    }
})
...
function changeStatus(btn, testId){
    const url = "/dashboard/tests/status/"+testId
...
function assignQuestions(designation, title, name, id){
    const url = "/dashboard/tests/"+id+"/assign/questions"
